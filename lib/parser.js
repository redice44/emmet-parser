...
function makeNode(tokens) {
  ...
  while (tokens.length > 0) {
    ...
    if (isOp(token)) {
      if (token[0] === opperators.GROUP_START) {
        stacks.node.push(processGrouping(token, tokens));
  ...
  // while (stacks.node.length > 1) {
  //   var child = stacks.node.pop();
  //   var parent = stacks.node.pop();
  //   stacks.node.push(evaluateChild(parent, child));
  // }
...
/**
  ...
  @return {DOM Node} - The DOM Node structure that this grouping creates.
*/
function processGrouping (token, tokens) {
  if (process.env.NODE_ENV !== 'production') {
  ...
  if (endIndex >= 0) {
    var groupedTokens = tokens.slice(0, endIndex + 1);
